...
export const createTable = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (!number || !capacity) {
      res.status(400).json({ success: false, message: "Number and capacity are required." });
      return;
    }

    const existingTable = await TableModel.findOne({ number });
    if (existingTable) {
      res.status(409).json({ success: false, message: "Table with this number already exists." });
      return;
    }
    ...
    res.status(201).json({ success: true, message: "Table created successfully.", data: savedTable });
  } catch (error) {
...
export const updateTable = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const updatedTable = await TableModel.findByIdAndUpdate(id, updates, { new: true });
    if (!updatedTable) {
